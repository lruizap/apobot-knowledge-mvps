...
app.MapPost("/api/chat",async(ChatRequest x,Store s,OllamaClient ollama,CancellationToken t)=>{
 ...
 var q=string.Join(" OR ",Regex.Matches(x.Message.ToLowerInvariant(),@"[\p{L}\p{N}]{2,}").Select(v=>v.Value));var chunks=await s.Chunks(q,5,t);
...
sealed class OllamaClient{
 readonly HttpClient h;public bool Available{get;private set;}public string Model=>Environment.GetEnvironmentVariable("OLLAMA_MODEL")??"qwen3:4b-instruct";
